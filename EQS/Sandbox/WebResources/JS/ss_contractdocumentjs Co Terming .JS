...
$(document).ready(function () {
        ...
        click: function () {
            ...
            if (tblLength > 0) {
                ...
                for (var i = 0; i < tblLength; i++) {
                    ...
                    autoRenewalVal = $(tr).find('td:eq(16)').find('select').children("option:selected").val();

                    oppStatus = $(tr).find('td:eq(17)').find('div:eq(0)').text();

                    isOppStsChanged = $(tr).find('td:eq(18)').find('input[type=checkbox]').prop('checked');
                    ...
                    //irf: comment below to remove columns from table
                    // var incresetype = $(tr).find('td:eq(9)').find('select').children("option:selected").val();
                    // if (incresetype !== "" && incresetype !== undefined && incresetype !== null) {
                    //     entity.ss_incresetype = incresetype;
                    // }


                    // entity.ss_increaseinpercentage = Number($(tr).find('td:eq(10)').find('input[type=number]').val());
                    // var incper = Number($(tr).find('td:eq(10)').find('input[type=number]').val());
                    // var incdate = $(tr).find('td:eq(11)').find('input[type=date]').val();
                    // if (incdate !== null && incdate !== "") {
                    //     entity.ss_increasedate = incdate;
                    // }
                    var currencyid = $(tr).find('td:last').html();
                    //25 to 22
                    var new_actualfee_value = $(tr).find('td:eq(22)').text();
                    createapprovalchk = $(tr).find('td:eq(22)').html();

                    //14 to 12
                    var contractowner = $(tr).find('td:eq(12)').text();
                    ...
                    // }
                        ...
                        //22 to 19
                        cancellationatdate = $(tr).find('td:eq(19)').find('input[type=date]').val();
                        ...
                        //23 to 20
                        cancellationtodate = $(tr).find('td:eq(20)').find('input[type=date]').val();
                        ...
                        //24 to 21
                        var checkboxes = $(tr).find('td:eq(21)').find('input[type=checkbox]:checked').map(function () {
                            return this.value;
                        }).get().join(',');
                        ...
                        if (confirm("Are you SURE?\nYou want to TERMINATE the CONTRACT !") == true) {
                            ...
                            //as td number 10 is removed (increaseinpercentage)
                ...
                $.ajax(
                    {
                        ...
                        success: function (data) {
                            ...
                            debugger;
                            // if (incresetype !== null && incresetype !== "" && incdate !== null && incdate !=="") {
                            //     CreatePriceIncrease(OppId, incper, incdate);
                            // }
                            //if(createapprovalchk === "true" || createapprovalchk === true) {
                                CreateTermination(cancellationatdate, cancellationtodate, checkboxes, OppId, contractnumber, contractowner, "account", "Contract", "", "", new_processtype, "", currencyid,new_actualfee_value);
                            //}
    ...
    function CreateTermination(cancellationatdate, cancellationtodate, checkboxes, OppId, contractnumber, contractowner, entitytype, items, licencefee, ss_productmaincategory, new_processtype, productid, currencyid,new_actualfee_value) {
        var entity = {};
        ...
        entity.new_actuallicensefee = new_actualfee_value !== "" || new_actualfee_value !== null || new_actualfee_value !== undefined ? parseFloat(new_actualfee_value) : null;
        ...
        if(currencyid !== null) {
            entity["ss_transactioncurrencyid@odata.bind"] = '/transactioncurrencies(' + currencyid.replace("{", "").replace("}", "") +')';
        }
        ...
        parent.Xrm.WebApi.online.createRecord("ss_approval", entity).then(
            ...
            function (error) {
                console.log("Error from creating termination process => " + error.message);
            ...
            $.ajax(
                {
                    ...
                    success: function (data) {
                        ...
                        $(".loader").fadeOut().delay(2000);
                        alert("The record has been updated");
                        CreateTermination(productcancellationatdate, productcancellationtodate, productcancelreason, OppId, "", "", "opportunityproduct", items, licencefee, ss_productmaincategory, new_processtype, proudctid,currencyid,new_actualfee_value);
        ...
        function success(results) {
            ...
            if (results.entities.length > 0) {
                for (var i = 0; i < results.entities.length; i++) {
                    ...
                    var actualfee = results.entities[i]["new_actualfee@OData.Community.Display.V1.FormattedValue"];
                    var actualfee_vale = results.entities[i]["new_actualfee"];
                    var date = "";

                    if (actualfee == null || actualfee == undefined) {
                        actualfee = "";
                    }
                    ...
                    productTable = productTable + "<tr class='productstr' id='" + opportunityproductid + "_pd' onchange='isProductValChange(this)'><td style='display:none;'>p" + opportunityID + "</td><td class='Prod-and-ContractName'>" + productname + "</td><td style='display:none;'>" + ss_setupfee_formatted + "</td><td>" + ss_monthlylicensefee_formatted + "</td>"
                        ...
                        + "<td>" + actualfee + "</td>"
                        + "<td id = 'golivedate' class='product_livedate' ><input type='date' id='golive' onchange='UpdateDate(this)' value = '" + date + "' " + displaycontractcolumns + "></td>"
                        ...
                        + "Other</label></span>"
                        + "<span class='d-block menu-option'><label><input type='checkbox' value= '11'>&nbsp;"
                        + "Cockpit Migration</label></span>"
                        + "<span class='d-block menu-option'><label><input type='checkbox' value= '12'>&nbsp;"
                        + "Formal Contract Changes</label></span>"
                        + "<span class='d-block menu-option'><label><input type='checkbox' value= '13'>&nbsp;"
                        + "Russia Shutdown</label></span>"
                        + "</div>"
                        + "</div>"
                    ...
                    productTable += "<td style='display:none;'>" + ss_productmaincategory_formatted + "</td><td style='display:none;'>" + ss_licensefee + "</td><td id = 'oppid' style='display:none;'>" + actualfee_vale + "</td><td style='display:none;'>" + productid + "</td></tr>";
  ...
  <entity name="opportunity">
    ...
    window.parent.Xrm.WebApi.online.retrieveMultipleRecords("opportunity", fetchXML2).then(
        function success(results) {
            ...
            myOppertutitiesList.forEach(element => {
                createContractDetailsRow(element.oppId, element.contracts, element.createdOn, element.ownerName, element.terminationTime, element.EndOfContract, element.renewal, element.contractid, element.priceincrease, element.ss_contractstatus, element.modifycontractstatus, element.cancellationatdate, element.cancellationtodate, element.contractcancelreason, element.ss_terminationperiod, element.contractowner, element.TL_annotationid, element.TL_filename, element.transfertofinance, element.contractnumber, element.ss_totallicencefee, element.salelsopportunityproductsobj, element.prdouctsname, element.ss_bcendofcontract, element.ss_currentyearpriceincrease, element.ss_revisionnumber, element.oppname, element.contractcancelreasonreason, element.new_actuallicensefee, element.currencyid, element.ss_createapprovalchecklist,element.new_actualfee_value);
            });
...
var notearray = [];
function createContractDetailsRow(oppid, contracts, createdOn, ownerName, terminationTime, EndOfContract, renewal, contractid, priceincrease, ss_contractstatus, modifycontractstatus, cancellationatdate, cancellationtodate, contractcancelreason, ss_terminationperiod, contractowner, TL_annotationid, TL_filename, transfertofinance, contractnumber, ss_totallicencefee, productsegment, prdouctsname, ss_bcendofcontract, ss_currentyearpriceincrease, ss_revisionnumber, oppname, contractcancelreasonformatted, new_actuallicensefee, currencyid,ss_createapprovalchecklist,new_actualfee_value) {
    ...
    var isModified;
    var Contractstatustext = "";
    if(ss_createapprovalchecklist === false && ss_contractstatus === "New"){
        setColor = "green";
        textColor = "white";
        Contractstatustext = "Active";
    }
    else if (ss_contractstatus === "New" && renewal === 0 && (new Date(EndOfContract) < new Date())) {
        setColor = "red";
        textColor = "white";
    ...
    var markup = "<tr class='app' id='" + oppid + "_CR' ><td class='align-middle'><input type='checkbox' class='expandTbl' ><input type='hidden' value=" + oppid + " id='oppidvalue'></td><td class='align-middle'><a class='fileLink' href=# onclick='OpenOpportunity(this)'> " + oppname + "</a></td><td class='align-middle maektextleft'>" + finalproductsegment + "</td><td class='align-middle maektextleft'>" + finalproductstodisplay + "</td><td class='align-middle'>" + ss_totallicencefee + "</td><td class='align-middle'>" + new_actuallicensefee + "</td><td style='width:15%;' class='Prod-and-ContractName align-middle'> " + allAnchorTags_Contracts + " </td><td class='align-middle'> " + contractnumber + "</td><td class='align-middle'>" + ss_revisionnumber + "</td><td class='align-middle'><button type='button' class='btn btn-primary btn-xs' onclick='FetchOppData(this)'>Start Cross-Selling</button></td><td class='align-middle'>" + createdOn + "</td><td class='align-middle'>" + ownerName + "</td><td class='align-middle'>" + contractowner + "</td><td class='align-middle'>" + ss_terminationperiod +
        ...
        + "<span class='d-block menu-option'><label style='float:left'><input type='checkbox' value= '12'>&nbsp;"
        + "Formal Contract Changes</label></span><br>"
        + "<span class='d-block menu-option'><label style='float:left'><input type='checkbox' value= '13'>&nbsp;"
        + "Russia Shutdown</label></span><br>"
        + "</div>"
        + "</div>"
        + "<div class='d-none' id='overlay' onclick='hide(event)'></div></td>"
    ...
    var markup2 = "<tr class='OppProducts' style='display:none;'><td class='class='text-center' colspan='16' >"
        ...
        + "<th>Yearly Lisence</th><th id ='liveproduct'>Actual Fee (BC)</th><th id ='liveproduct'>Go Live Date</th><th>Product Status</th><th>Up-Sell</th><th style='display:none;'>Down-Sell</th>"
...
function CreateCrossSellingOpp(_ownerid_value, _parentaccountid_value, _pricelevelid_value, _transactioncurrencyid_value, id, _parentcontactid_value, opportunitysell, new_productcategory) {
    ...
    if (_parentcontactid_value != null) {
...
function UpdateDate(ProductDate) {
    $(".loader").fadeIn();
    var recrodid = $(ProductDate).closest('tr').attr('id');
    recrodid = recrodid.substring(0, recrodid.length - 3);
    var date = $(ProductDate).val();
    console.log("date => " + date);
    var record = {};
    record.ss_productlivedate = new Date(date).toISOString();; // Date Time

    parent.Xrm.WebApi.updateRecord("opportunityproduct", recrodid, record).then(
        function success(result) {
