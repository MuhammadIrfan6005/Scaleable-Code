...
$(document).ready(function () {
        ...
        click: function () {
            ...
            if (tblLength > 0) {
                ...
                for (var i = 0; i < tblLength; i++) {
                    ...
                    autoRenewalVal = $(tr).find('td:eq(17)').find('select').children("option:selected").val();

                    oppStatus = $(tr).find('td:eq(18)').find('div:eq(0)').text();

                    isOppStsChanged = $(tr).find('td:eq(19)').find('input[type=checkbox]').prop('checked');

                    var contractnumber = $(tr).find('td:eq(5)').text();

                    entity.ss_revisionnumber = $(tr).find('td:eq(6)').text();
                    ...
                    var incresetype = $(tr).find('td:eq(7)').find(":selected").val();
                    entity.ss_incresetype = incresetype;

                    entity.ss_increaseinpercentage = Number($(tr).find('td:eq(8)').find('input[type=number]').val());
                    var incper = Number($(tr).find('td:eq(9)').find('input[type=number]').val());;
                    entity.ss_increasedate = $(tr).find('td:eq(9)').find('input[type=date]').val();
                    ...
                    // }
                        ...
                        cancellationatdate = $(tr).find('td:eq(20)').find('input[type=date]').val();
                        ...
                        cancellationtodate = $(tr).find('td:eq(21)').find('input[type=date]').val();
                        ...
                        var checkboxes = $(tr).find('td:eq(22)').find('input[type=checkbox]:checked').map(function () {
                            return this.value;
                        }).get().join(',');
    ...
    function CreateTermination(cancellationatdate, cancellationtodate, checkboxes, OppId, contractnumber, contractowner, entitytype, items, licencefee, ss_productmaincategory, productid) {
        var entity = {};
            ...
            $.ajax(
                {
                    ...
                    success: function (data) {
                        ...
                        $(".loader").fadeOut().delay(2000);
                        alert("The record has been updated");
                        CreateTermination(productcancellationatdate, productcancellationtodate, productcancelreason, OppId, "", "", "opportunityproduct", items, licencefee, ss_productmaincategory, proudctid);
            ...
            if (results.entities.length > 0) {
                for (var i = 0; i < results.entities.length; i++) {
                    ...
                    productTable = productTable + "<tr class='productstr' id='" + opportunityproductid + "_pd' onchange='isProductValChange(this)'><td style='display:none;'>p" + opportunityID + "</td><td class='Prod-and-ContractName'>" + productname + "</td><td style='display:none;'>" + ss_setupfee_formatted + "</td><td>" + ss_monthlylicensefee_formatted + "</td>"
                        ...
                        + "<div class='OppProductStatus' style='color:" + textColor + ";background-color:" + setColor + "'>" + productstatus + "</div></td><td class='align-middle'><button type='button' class='btn btn-primary btn-xs UP' onclick='FetchOppData(this)'>Start Up-Sell</button></td><td class='align-middle'><button type='button' class='btn btn-primary btn-xs DOWN' onclick='FetchOppData(this)'>Start Down-Sell</button></td><td><div class='dropdown'>"
  ...
  <entity name="opportunity">
    ...
    window.parent.Xrm.WebApi.online.retrieveMultipleRecords("opportunity", fetchXML2).then(
        function success(results) {
            ...
            for (var i = 0; i < results.entities.length; i++) {
                ...
                if (results.entities[i]["ano.notetext"] !== "{{Letter--") {
                    ...
                    if (!checkOppidArray.includes(oppID)) {
                        ...
                        myCurrentobj.EndOfContract = (endDate == undefined || endDate == null) ? "" : moment(endDate.substring(0, 10)).format("DD-MM-YYYY");
                        ...
                        myCurrentobj.ss_increasedate = (increasedate !== undefined && increasedate !== null) ? new Date(increasedate).toISOString().substring(0, 10) : new Date().toISOString().substring(0, 10);
            ...
            myOppertutitiesList.forEach(element => {
                createContractDetailsRow(element.oppId, element.contracts, element.createdOn, element.ownerName, element.terminationTime, element.EndOfContract, element.renewal, element.contractid, element.priceincrease, element.ss_contractstatus, element.modifycontractstatus, element.ss_incresetype, element.cancellationatdate, element.cancellationtodate, element.contractcancelreason, element.ss_terminationperiod, element.contractowner, element.TL_annotationid, element.TL_filename, element.transfertofinance, element.contractnumber, element.ss_totallicencefee, element.salelsopportunityproductsobj, element.prdouctsname, element.ss_bcendofcontract, element.ss_currentyearpriceincrease, element.ss_increaseinpercentage, element.ss_increasedate, element.ss_revisionnumber);
            });
...
function createContractDetailsRow(oppid, contracts, createdOn, ownerName, terminationTime, EndOfContract, renewal, contractid, priceincrease, ss_contractstatus, modifycontractstatus, ss_incresetype, cancellationatdate, cancellationtodate, contractcancelreason, ss_terminationperiod, contractowner, TL_annotationid, TL_filename, transfertofinance, contractnumber, ss_totallicencefee, productsegment, prdouctsname, ss_bcendofcontract, ss_currentyearpriceincrease, ss_increaseinpercentage, ss_increasedate, ss_revisionnumber) {
    ...
    if (ss_currentyearpriceincrease) {
        $('.increases').show();
    ...
    var markup = "<tr class='app' id='" + oppid + "_CR' ><td class='align-middle'><input type='checkbox' class='expandTbl' ><input type='hidden' value=" + oppid + " id='oppidvalue'></td><td class='align-middle maektextleft'>" + finalproductsegment + "</td><td class='align-middle maektextleft'>" + finalproducts + "</td><td class='align-middle'>" + ss_totallicencefee + "</td><td style='width:15%;' class='Prod-and-ContractName align-middle'> " + allAnchorTags_Contracts + " </td><td class='align-middle'> " + contractnumber + "</td><td class='align-middle'>" + ss_revisionnumber + "</td><td><select class='btn-outline-info incresetype' onchange='CurrentYearPI()'> <option disabled selected value> - select an option - </option><option value='1' "+(ss_incresetype === "1" ? "selected" :"") +">Danish Index</option><option value='2'  "+(ss_incresetype === "2" ? "selected" :"") +">French Index</option><option value='3'  "+(ss_incresetype === "3" ? "selected" :"") +">German Index</option><option value='4'  "+(ss_incresetype === "4" ? "selected" :"") +">Italian Index</option><option value='5'  "+(ss_incresetype === "5" ? "selected" :"") +">Individual Value</option></select></td><td class='align-middle' ><input type='number' class='increasesTD' id='increaseper' name='quantity' min='1' max='5' value ='" + ss_increaseinpercentage + "' " + displayincreasecolumns + " /></td><td class='align-middle' ><input type='date' class='increasesTD' value = '" + ss_increasedate + "' " + displayincreasecolumns + "></td><td class='align-middle'><button type='button' class='btn btn-primary btn-xs' onclick='FetchOppData(this)'>Start Cross-Selling</button></td><td class='align-middle'>" + createdOn + "</td><td class='align-middle'>" + ownerName + "</td><td class='align-middle'>" + contractowner + "</td><td class='align-middle'>" + ss_terminationperiod +
    ...
    //irf: by default showing saved incresetype and enabling disabling and show hide class "increasesTD"
    if (ss_incresetype == "") {
        console.log("ss_incresetype is empty" + ss_incresetype);
    }
    else {
        
        console.log("ss_incresetype is not empty" + ss_incresetype);
        // $('.incresetype ').val(ss_incresetype);
        $(".increasesTD:first").val(ss_increaseinpercentage);
        var selectedIncreseTypeis = $(".incresetype").children("option:selected").val();
        if (selectedIncreseTypeis != "") {
            $(".incresetype").closest('tr').find('.increasesTD').show();
            if (selectedIncreseTypeis != 5) {
                $(".increasesTD").attr('readonly', 'readonly');
                $(".increasesTD").css('background-color', '#A9A9A9');
            }
            else {
                $('.increasesTD').attr('readonly', false);
                $(".increasesTD").css('background-color', 'white');
            }
        }
        else{
            $(".incresetype").closest('tr').find('.increasesTD').hide();
        }
...
function CurrentYearPI() {
    ...
    var selectedIncreseType = $(".incresetype").children("option:selected").val();
    parent.Xrm.WebApi.online.retrieveMultipleRecords("ss_navisionidmapping", "?$select=new_danishindex,new_frenchindex,new_germanindex,new_italianindex").then(
        function success(results) {
            ...
            if (selectedIncreseType != "") {
                $(".incresetype").closest('tr').find('.increasesTD').show();
                if (selectedIncreseType == "1") {
                    $(".increasesTD:first").val(new_danishindex);
                }
                else if (selectedIncreseType == "2") {
                    $(".increasesTD:first").val(new_frenchindex);
                }
                else if (selectedIncreseType == "3") {
                    $(".increasesTD:first").val(new_germanindex);
                }
                else if (selectedIncreseType == "4") {
                    $(".increasesTD:first").val(new_italianindex);
                }
                else if (selectedIncreseType == "5") {
                    $(".increasesTD:first").val(0);
                }
...
function FetchOppData(oppobj, recID) {
    $(".loader").fadeIn();
    var id = "";
    if (recID !== "" || recID !== null) {
        id = recID;
    ...
    parent.Xrm.WebApi.online.retrieveRecord("opportunity", id, "?$select=_ownerid_value,_parentaccountid_value,_pricelevelid_value,ss_contractnumber,_transactioncurrencyid_value").then(
        function success(result) {
            ...
            CreateCrossSellingOpp(_ownerid_value, _parentaccountid_value, _pricelevelid_value, ss_contractnumber, _transactioncurrencyid_value, id);
        },
        function (error) {
            parent.Xrm.Utility.alertDialog(error.message);
        }
...
function CreateCrossSellingOpp(_ownerid_value, _parentaccountid_value, _pricelevelid_value, ss_contractnumber, _transactioncurrencyid_value, id) {
    var entity = {};
