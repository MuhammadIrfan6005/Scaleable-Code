function ShowHideSectionJira(executionContext) {
    ...
    var new_issuetype = formContext.getAttribute("new_mapping").getValue();
    var issuetype = "";
    if (new_issuetype !== null) {
        ...
        if (issuetype === "Policy Manager" || issuetype === "Approval Manager" || issuetype === "Single Sign-On" || issuetype === "Data Sync") {
            ...
            formContext.ui.tabs.get("jira_template_tab").sections.get("jira_template_section_1").setVisible(false);
            formContext.ui.tabs.get("jira_template_tab").sections.get("jira_template_section_2").setVisible(false);
	    formContext.ui.tabs.get("jira_template_tab").sections.get("jira_template_section_3").setVisible(false);
        }
	    ...
	    //formContext.ui.tabs.get("jira_template_tab").sections.get("jira_template_section_2").setVisible(false);
            ...
            //formContext.getControl("new_gotethicspackage").setVisible(false);s
        }
    }
}
function makegeneralfieldreadonly(formContext) {
    formContext.getControl("new_mapping").setDisabled(true);
