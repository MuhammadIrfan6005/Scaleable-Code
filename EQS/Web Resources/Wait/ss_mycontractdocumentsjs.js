...
$("#cdon").click(function(){
   ...
   else if(elmClass === "desc")
   {
    ...
    RetrieveContractDetailsTblData(sortByCreatedOn, otherDateFilters, otherCountryFilters, otherAccountTitleFilters, false)
   ...
   click: function () {
       ...
       if (tblLength > 0) {
           ...
           for (var i = 0; i < tblLength; i++) {
               ...
               oppStatus = $(tr).find('td:eq(7)').find('div:eq(0)').text();
...
function RetrieveContractDetailsTblData(sortByCreatedOn, dateRangeFilter, countryFilter, accounttitleFilter, isFirstcall) {
...
window.parent.Xrm.WebApi.online.retrieveMultipleRecords("opportunity", fetchXML2).then(
   function success(results) {
      ...
      $("#backToHome").hide();
      $("#trecordsText").empty();
      $("#trecordsText").text("Total no of Contracts : " + checkArray.length);    
      SortByDescContractDetailsTblData(MainRecordsArray, sortByCreatedOn);
...
//nh=========================================
//=============================================nh
function createContractDetailsRow(accountid, bbo_navid, _parentaccount, oppid, contractName, createdOn, ownerName, country, EndOfContract, renewal, contractid, ss_contractstatus, modifycontractstatus, annotationid) {
   //nh===============
...
function SortByDescContractDetailsTblData(MainRecordsArray, sortByCreatedOn) {
    ...
    for (var i = 0; i < MainRecordsArray.length; i++) {
               ...
               if (ss_contractstatus == undefined) {
                ...
                createContractDetailsRow(accountid, bbo_navid, _parentaccount, oppID, name, createdOn, ownerName, country, endDate, autoRenewal, i, ss_contractstatus, modifycontractstatus, annotationid);
