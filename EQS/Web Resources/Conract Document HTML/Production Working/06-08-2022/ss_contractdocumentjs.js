...
$(document).ready(function () {
        ...
        click: function () {
            ...
            if (tblLength > 0) {
                ...
                for (var i = 0; i < tblLength; i++) {
                    ...
                    autoRenewalVal = $(tr).find('td:eq(9)').find('select').children("option:selected").val();

                    oppStatus = $(tr).find('td:eq(10)').find('div:eq(0)').text();

                    isOppStsChanged = $(tr).find('td:eq(11)').find('input[type=checkbox]').prop('checked');
                    var contractnumber = $(tr).find('td:eq(2)').text();

                    var contractowner = $(tr).find('td:eq(6)').text();
                    ...
                    // }
                        ...
                        cancellationatdate = $(tr).find('td:eq(12)').find('input[type=date]').val();
                        ...
                        cancellationtodate = $(tr).find('td:eq(13)').find('input[type=date]').val();
                        ...
                        var checkboxes = $(tr).find('td:eq(14)').find('input[type=checkbox]:checked').map(function () {
                            return this.value;
                        }).get().join(',');
    ...
    function CreateTermination(cancellationatdate, cancellationtodate, checkboxes, OppId, contractnumber, contractowner, entitytype, items, licencefee, ss_productmaincategory, new_processtype) {
        var entity = {};
        ...
        entity.ss_contractnumber = contractnumber !== null ? contractnumber : "";
        entity.ss_officialcontractpartner = contractowner !== null ? contractowner : "";
            ...
            $.ajax(
                {
                    ...
                    success: function (data) {
                        ...
                        $(".loader").fadeOut().delay(2000);
                        alert("The record has been updated");
                        CreateTermination(productcancellationatdate, productcancellationtodate, productcancelreason, OppId, "", "", "opportunityproduct", items, licencefee, ss_productmaincategory, new_processtype);
...
function UpdateContractStatus(contractID, statusVal) {
    ...
    if (statusVal) {
        ...
        var status = [];
        status = $(trclass).next().find('.productstr').find('td:eq(9)').find('input:checkbox:not(:checked)');

        for (var i = 0; i < status.length; i++) {
            //$(trclass).next().find('.productstr').find('td:eq(9)').find('input[type=checkbox]').prop("checked", true);
            $(status[i]).prop("checked", true);
        }
                $(trclass).next().find('.product_contractcolumn').show();
          //$(trclass).next().find('.removetable').hide();
          var val = $(trclass).next().find('.prod_cancel_at');
          for (l = 0; l < val.length; l++) {
              var x = $(val[l]);
              if (!x.val()) {
                  x.val(contract_cancelatdate);
          ...
          var dateVal = $(trclass).next().find('.prod_cancel_to');
          for (j = 0; j < dateVal.length; j++) {
              var y = $(dateVal[j]);
              if (!y.val()) {
                  y.val(contract_canceltodate);
            ...
            if (results.entities.length > 0) {
                for (var i = 0; i < results.entities.length; i++) {
                    ...
                    productTable = productTable + "<tr class='productstr' id='" + opportunityproductid + "_pd' onchange='isProductValChange(this)'><td style='display:none;'>p" + opportunityID + "</td><td class='Prod-and-ContractName'>" + productname + "</td><td style='display:none;'>" + ss_setupfee_formatted + "</td><td>" + ss_monthlylicensefee_formatted + "</td>"
                        + "<td>" + ss_licensefee_formatted + "</td><td>" + ss_startdate + "</td><td>" + ss_termenddate + "</td>"
                        ...
                        + "Reason &#9013;</button>"
                        + "<div class='d-none shadow rounded menu' style='float:left'>"
...
function RetrieveContractDetailsTblData() {
    ...
    var fetchXML2 = `<fetch version="1.0" output-format="xml-platform" mapping="logical">
                       <entity name="opportunity">
                            ...
                            <attribute name="opportunityid" /> 
                            <attribute name="ss_newpriceincreases" />
    ...
    window.parent.Xrm.WebApi.online.retrieveMultipleRecords("opportunity", fetchXML2).then(
        function success(results) {
            ...
            myOppertutitiesList.forEach(element => {
                createContractDetailsRow(element.oppId, element.contracts, element.createdOn, element.ownerName, element.terminationTime, element.EndOfContract, element.renewal, element.contractid, element.priceincrease, element.ss_contractstatus, element.modifycontractstatus, element.cancellationatdate, element.cancellationtodate, element.contractcancelreason, element.ss_terminationperiod, element.contractowner, element.TL_annotationid, element.TL_filename, element.transfertofinance, element.contractnumber);
            });
...
function createContractDetailsRow(oppid, contracts, createdOn, ownerName, terminationTime, EndOfContract, renewal, contractid, priceincrease, ss_contractstatus, modifycontractstatus, cancellationatdate, cancellationtodate, contractcancelreason, ss_terminationperiod, contractowner, TL_annotationid, TL_filename, transfertofinance, contractnumber) {
    ...
    var markup = "<tr class='app' id='" + oppid + "_CR' ><td class='align-middle'><input type='checkbox' class='expandTbl' ><input type='hidden' value=" + oppid + " id='oppidvalue'></td><td style='width:15%;' class='Prod-and-ContractName align-middle'> " + allAnchorTags_Contracts + " </td><td class='align-middle'> " + contractnumber + "</td><td class='align-middle'>" + priceincrease + "</td><td class='align-middle'>" + createdOn + "</td><td class='align-middle'>" + ownerName + "</td><td class='align-middle'>" + contractowner + "</td><td class='align-middle'>" + ss_terminationperiod +
        ...
        + EndOfContract + "</td><td class='align-middle'><select class='btn-outline-info'  id='" + oppid + "' onchange='isContractValChange(this)' >"
    ...
    var markup2 = "<tr class='OppProducts' style='display:none;'><td class='class='text-center' colspan='16' >"
        ...
        + "<th>Yearly Lisence</th><th>Term Start Date</th><th>Term End Date</th><th id ='liveproduct'>Go Live Date</th><th>Product Status</th>"
