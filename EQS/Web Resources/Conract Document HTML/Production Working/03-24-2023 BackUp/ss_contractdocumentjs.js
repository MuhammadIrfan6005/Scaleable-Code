...
$(document).ready(function () {
                ...
                $.ajax(
                    {
                        ...
                        success: function (data) {
                            ...
                            CreateTermination(cancellationatdate, cancellationtodate, checkboxes, OppId, contractnumber, contractowner, "account", "Contract", "", "", new_processtype, "", currencyid);
    ...
    function CreateTermination(cancellationatdate, cancellationtodate, checkboxes, OppId, contractnumber, contractowner, entitytype, items, licencefee, ss_productmaincategory, new_processtype, productid, currencyid) {
        var entity = {};
            ...
            $.ajax(
                {
                    ...
                    success: function (data) {
                        ...
                        $(".loader").fadeOut().delay(2000);
                        alert("The record has been updated");
                        CreateTermination(productcancellationatdate, productcancellationtodate, productcancelreason, OppId, "", "", "opportunityproduct", items, licencefee, ss_productmaincategory, new_processtype, proudctid, currencyid);
  ...
  <entity name="opportunity">
    ...
    window.parent.Xrm.WebApi.online.retrieveMultipleRecords("opportunity", fetchXML2).then(
        function success(results) {
            ...
            myOppertutitiesList.forEach(element => {
                createContractDetailsRow(element.oppId, element.contracts, element.createdOn, element.ownerName, element.terminationTime, element.EndOfContract, element.renewal, element.contractid, element.priceincrease, element.ss_contractstatus, element.modifycontractstatus, element.ss_incresetype, element.cancellationatdate, element.cancellationtodate, element.contractcancelreason, element.ss_terminationperiod, element.contractowner, element.TL_annotationid, element.TL_filename, element.transfertofinance, element.contractnumber, element.ss_totallicencefee, element.salelsopportunityproductsobj, element.prdouctsname, element.ss_bcendofcontract, element.ss_currentyearpriceincrease, element.ss_increaseinpercentage, element.ss_increasedate, element.ss_revisionnumber, element.oppname, element.contractcancelreasonreason, element.new_actuallicensefee, element.currencyid);
            });
...
function CurrentYearPI(mscobj) {
    ...
    parent.Xrm.WebApi.online.retrieveMultipleRecords("ss_navisionidmapping", "?$select=new_danishindex,new_frenchindex,new_germanindex,new_italianindex,new_spanishindex").then(
        function success(results) {
            for (var i = 0; i < results.entities.length; i++) {
