        ...
        click: function () {
            ...
            if (tblLength > 0) {
                ...
                for (var i = 0; i < tblLength; i++) {
                    ...
                    //irf: getting new actuallicencefee to saveinto db
                    new_actuallicensefee = $(tr).find('td:eq(5)').find('span:first').text();
                ...
                $.ajax(
                    {
                        ...
                        success: function (data) {
                            ...
                            CreatePriceIncrease(OppId, incdate, reason, ss_totallicencefee, exceptionalIncreasePercentage, new_actuallicensefee, comment, childtableposition);
                        },
...
function updateincreasetype(id, incdateIS, selectedIncreseType, ss_totallicencefee, increaseper, new_actuallicensefee, childtableposition) {
    var comment = "Standard Price Increase";
    CreatePriceIncrease(id, incdateIS, selectedIncreseType, ss_totallicencefee, increaseper, new_actuallicensefee, comment, childtableposition );
    var entity = {};
...
function CreatePriceIncrease(OppId, last_price_increase, Increase_type, contract_start_arr, Increse_in_percen, new_actuallicensefee, comment, childtableposition) {
    var entity = {};
    entity["ss_opportunity@odata.bind"] = "/opportunities(" + OppId + ")";
    // irf: adding data into below fields 
    entity.ss_lastpriceincrease = new Date(last_price_increase).toISOString();
    ...
    var ss_latestarr= Number(parseFloat(new_actuallicensefee).toFixed(2));
    entity.ss_latestarr = ss_latestarr;
    //irf: check if priceincrease current table rows are less then 1 then seperate logic, if 1 or greater then seperate logic
    rowCount = $('.priceIncreasetbl:eq("'+ childtableposition +'") >tbody >tr').length;
    if (rowCount == 0) {

    var increaseValue = contract_start_arr - ss_latestarr;
    entity.ss_increasevalue = increaseValue;

    } else {
        var last_ss_latestarr = $('.priceIncreasetbl:eq("' + childtableposition + '") tr:last').find('td:eq(4)').find("span").text()
        entity.ss_increasevalue = last_ss_latestarr - ss_latestarr;
    }
    entity.ss_comment = comment;
...
function createContractDetailsRow(ss_reason, ss_exceptionalincrease, ss_exceptionalincreaseinpercentage, oppid, contracts, createdOn, ownerName, terminationTime, EndOfContract, renewal, contractid, priceincrease, ss_contractstatus, modifycontractstatus, ss_incresetype, cancellationatdate, cancellationtodate, contractcancelreason, ss_terminationperiod, contractowner, TL_annotationid, TL_filename, transfertofinance, contractnumber, ss_totallicencefee, productsegment, prdouctsname, ss_bcendofcontract, ss_currentyearpriceincrease, ss_increaseinpercentage, ss_increasedate, ss_revisionnumber, oppname, contractcancelreasonformatted, new_actuallicensefee, new_actuallicensefeeDecimal, currencyid, ss_createapprovalchecklist, new_actualfee_value) {
    ...
    var displayExceptionalIncreasecolumns = " ";
    if (ss_exceptionalincrease === true) {
        ss_exceptionalincrease = "checked";
    }
    else {
        ss_exceptionalincrease = "";
        displayExceptionalIncreasecolumns = "style='display:none'";
    }
    ...
    var markup = "<tr class='app' id='" + oppid + "_CR' ><td class='align-middle'><input type='checkbox' class='expandTbl' ><input type='hidden' value=" + oppid + " id='oppidvalue'></td><td class='align-middle'><a class='fileLink' href=# onclick='OpenOpportunity(this)'> " + oppname + "</a></td><td class='align-middle maektextleft'>" + finalproductsegment + "</td><td class='align-middle maektextleft'>" + finalproductstodisplay + "</td><td class='align-middle ss_totallicencefee'>" + ss_totallicencefee + "</td><td class='align-middle new_actuallicensefee'>" + new_actuallicensefee + "<span style='display:none'>" + new_actuallicensefeeDecimal + "</span></td><td style='width:15%;' class='Prod-and-ContractName align-middle'> " + allAnchorTags_Contracts + " </td><td class='align-middle'> " + contractnumber + "</td><td class='align-middle'>" + ss_revisionnumber + "</td><td class='align-middle'><select class='btn-outline-info incresetype' onchange='CurrentYearPI(this)'> <option disabled selected value> - select an option - </option><option value='1' " + (ss_incresetype === 1 ? "selected" : "") + ">Danish Index</option><option value='2'  " + (ss_incresetype === 2 ? "selected" : "") + ">French Index</option><option value='3'  " + (ss_incresetype === 3 ? "selected" : "") + ">German Index</option><option value='4'  " + (ss_incresetype === 4 ? "selected" : "") + ">Italian Index</option><option value='6'  " + (ss_incresetype === 6 ? "selected" : "") + ">Spanish Index</option><option value='7'  " + (ss_incresetype === 7 ? "selected" : "") + ">Swiss Index</option><option value='8'  " + (ss_incresetype === 8 ? "selected" : "") + ">US Index</option><option value='9'  " + (ss_incresetype === 9 ? "selected" : "") + ">UK Index</option><option value='5'  " + (ss_incresetype === 5 ? "selected" : "") + ">Individual Value</option></select></td><td class='align-middle'><input type='number' onchange='CurrentYearPI(this)' class='increasesTD' id='increaseper' name='quantity' min='1' max='5' value ='" + ss_increaseinpercentage + "' " + displayincreasecolumns + " " + fields + " /></td><td class='align-middle' ><input type='date' class='increasesTD' value = '" + ss_increasedate + "' " + displayincreasecolumns + " onchange='isContractValChange(this)'></td><td class='align-middle'>" + createdOn + "</td><td class='align-middle'>" + ownerName + "</td><td class='align-middle'>" + contractowner + "</td><td class='align-middle'>" + ss_terminationperiod +
        ...
        + "<td class='hideExceptionalFields' "+ displayExceptionalIncreasecolumns +">" + ss_reason + "</td><td class='hideExceptionalFields' "+ displayExceptionalIncreasecolumns +">" + ss_exceptionalincreaseinpercentage + "</td>"
        ...
        function success(results) {
            ...
            if (results.entities.length > 0) {
                for (var i = 0; i < results.entities.length; i++) {
                    ...
                    var ss_latestarr = result["ss_latestarr"]
                    var ss_latestarr_formated = result["ss_latestarr@OData.Community.Display.V1.FormattedValue"]
                    ...
                    priceIncreaseTable = priceIncreaseTable + "<tr class='productstr' id='" + ss_priceincreasehistoryid + "_pd' onchange='isProductValChange(this)'>"
                        ...
                        + "<td>" + ss_latestarr_formated + " <span style='display:none'>"+ ss_latestarr +"</span> </td>"
...
function CurrentYearPI(mscobj) {
    ...
    parent.Xrm.WebApi.online.retrieveMultipleRecords("ss_navisionidmapping", "?$select=new_danishindex,new_frenchindex,new_germanindex,new_italianindex,new_spanishindex,ss_swissindex,ss_usindex,ss_ukindex").then(
        function success(results) {
            ...
            ss_totallicencefee = parseFloat(parseFloat(ss_totallicencefee).toFixed(2));
            //irf: find actuallicensefee from span for latest_arr field into priceincrease
            var new_actuallicensefee = $(mscobj).closest('tr').find('td:eq(5)').find('span:first').text();
            updateincreasetype(id, incdateIS, selectedIncreseType, ss_totallicencefee, increaseper, new_actuallicensefee, childtableposition);
        },
        function (error) {
            Xrm.Utility.alertDialog(error.message);
        }
