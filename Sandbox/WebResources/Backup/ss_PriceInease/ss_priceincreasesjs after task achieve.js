...
$(document).ready(function () {
        ...
        click: function () {
            ...
            if (tblLength > 0) {
                ...
                for (var i = 0; i < tblLength; i++) {
                    ...
                    // incresetype = $(tr).find('td:eq(9)').find('select').children("option:selected").val();
                    ...
                    if (incdate !== null && incdate !== "") {
                        // irf: if next allowed increse date is not today then alert user and stop perticular record from save
                ...
                $.ajax(
                    {
                        ...
                        success: function (data) {
                            ...
                            // if (incresetype !== undefined && comment !== undefined && !isNaN(exceptionalIncreasePercentage)) {
    ...
    //     if ($(this).is(':checked')) {
    //       $('.popover-content').fadeIn(200);
    //     } else {
    //       $('.popover-content').fadeOut(200);
    //     }
    //   });
});
...
function CreatePriceIncrease(OppId, last_price_increase, Increase_type, contract_start_arr, Increse_in_percen, comment) {
    ...
    entity.ss_increasetype = Increase_type; // Choice
    entity.ss_contractstartarr = contract_start_arr; // Decimal
    entity.ss_increaseinpercentage = Increse_in_percen; // Decimal
    entity.ss_comment = comment; // Multiline Text

    parent.Xrm.WebApi.online.createRecord("ss_priceincreasehistory", entity).then(
        function success(result) {
            var newEntityId = result.id;
            console.log("newEntityId" + newEntityId);
            // irf: to reload complete opportunity table
            loadData();
        },
        function (error) {
            parent.Xrm.Utility.alertDialog(error.message);
        ...
        function success(results) {
            ...
            if (results.entities.length > 0) {
                for (var i = 0; i < results.entities.length; i++) {
                    ...
                    productTable = productTable + "<tr class='productstr' id='" + opportunityproductid + "_pd' onchange='isProductValChange(this)'><td'>   </td><td>    </td>"
                        + "<td>   </td>"
                        + "<td>   </td>"
                        + "<td>   </td>"
                        + "<td>   </td>"
    ...
    window.parent.Xrm.WebApi.online.retrieveMultipleRecords("opportunity", fetchXML2).then(
        function success(results) {
            ...
            for (var i = 0; i < results.entities.length; i++) {
                ...
                // irf: getting reason, comment, exceptionalincrese field
                ...
                if (ss_reason != undefined && ss_comment != undefined && ss_exceptionalincreaseinpercentage != undefined) {
                    // $("#cancelledtodate").show();
                    // $("#cancelreason").show();
                }
...
function createContractDetailsRow(ss_reason, ss_exceptionalincrease, ss_exceptionalincreaseinpercentage, oppid, contracts, createdOn, ownerName, terminationTime, EndOfContract, renewal, contractid, priceincrease, ss_contractstatus, modifycontractstatus, ss_incresetype, cancellationatdate, cancellationtodate, contractcancelreason, ss_terminationperiod, contractowner, TL_annotationid, TL_filename, transfertofinance, contractnumber, ss_totallicencefee, productsegment, prdouctsname, ss_bcendofcontract, ss_currentyearpriceincrease, ss_increaseinpercentage, ss_increasedate, ss_revisionnumber, oppname, contractcancelreasonformatted, new_actuallicensefee, currencyid, ss_createapprovalchecklist, new_actualfee_value) {
    ...
    if (ss_contractstatus !== "Cancellation") {
        ...
        $("#cancelledatdate").show();
        // irf: auto hide reason and Exception Increase In %	
        // $("#cancelledtodate").show();
        // $("#cancelreason").show();
        $("#upload_termination").show();
        readonly_fields = "readonly";
    ...
    // irf: show "" if ss_reason or ss_exceptionalincreaseinpercentage are empty
...
function getPriceIncrease(opportunityID) {
    ...
    var fetchXml = [
        ...
        "    <attribute name='ss_comment'/>",
        "    <filter>",
        "      <condition attribute='ss_opportunity' operator='eq' value='", fetchData.ss_opportunity/*fd8e163a-6cb6-ed11-83ff-0022489c9ca3*/, "'/>",
        "    </filter>",
        "  </entity>",
        "</fetch>"
    ].join("");
        ...
        function success(results) {
            ...
            if (results.entities.length > 0) {
                for (var i = 0; i < results.entities.length; i++) {
                    ...
                    // Columns
                    var ss_priceincreasehistoryid = result["ss_priceincreasehistoryid"]; // Guid
                    var ss_comment = result["ss_comment"]; // Multiline Text
                    var ss_contractstartarr = result["ss_contractstartarr"]; // Decimal
                    var ss_contractstartarr_formatted = result["ss_contractstartarr@OData.Community.Display.V1.FormattedValue"];
                    var ss_increaseinpercentage = result["ss_increaseinpercentage"]; // Decimal
                    var ss_increaseinpercentage_formatted = result["ss_increaseinpercentage@OData.Community.Display.V1.FormattedValue"];
                    var ss_increasetype = result["ss_increasetype"]; // Choice
                    var ss_increasetype_formatted = result["ss_increasetype@OData.Community.Display.V1.FormattedValue"];
                    var ss_increasevalue = result["ss_increasevalue"]; // Decimal
                    ...
                    var ss_lastpriceincrease = result["ss_lastpriceincrease"]; // Date Time
                    var ss_lastpriceincrease_formatted = result["ss_lastpriceincrease@OData.Community.Display.V1.FormattedValue"];
                    var ss_latestarr_base = result["ss_latestarr_base"]; // Currency
...
function setTwoNumberDecimal(event) {
    ...
    $(".exceptionalIncreasePercentage").val(exceptionalIncresePercentage);
}
