  ...
  <entity name="opportunity">
    ...
    window.parent.Xrm.WebApi.online.retrieveMultipleRecords("opportunity", fetchXML2).then(
        function success(results) {
            ...
            myOppertutitiesList.forEach(element => {
                createContractDetailsRow(element.oppId, element.contracts, element.createdOn, element.ownerName, element.terminationTime, element.EndOfContract, element.renewal, element.contractid, element.priceincrease, element.ss_contractstatus, element.modifycontractstatus, element.ss_incresetype, element.cancellationatdate, element.cancellationtodate, element.contractcancelreason, element.ss_terminationperiod, element.contractowner, element.TL_annotationid, element.TL_filename, element.transfertofinance, element.contractnumber, element.ss_totallicencefee, element.salelsopportunityproductsobj, element.prdouctsname, element.ss_bcendofcontract, element.ss_currentyearpriceincrease, element.ss_increaseinpercentage, element.ss_increasedate, element.ss_revisionnumber, element.oppname);
            });
...
function createContractDetailsRow(oppid, contracts, createdOn, ownerName, terminationTime, EndOfContract, renewal, contractid, priceincrease, ss_contractstatus, modifycontractstatus, ss_incresetype, cancellationatdate, cancellationtodate, contractcancelreason, ss_terminationperiod, contractowner, TL_annotationid, TL_filename, transfertofinance, contractnumber, ss_totallicencefee, productsegment, prdouctsname, ss_bcendofcontract, ss_currentyearpriceincrease, ss_increaseinpercentage, ss_increasedate, ss_revisionnumber, oppname) {
    ...
    var fields = "readonly";
    if (ss_incresetype !== null && ss_incresetype !== "") {
        $('.increases').show();
        ...
        function success(results) {
            ...
            if (selectedIncreseType !== "") {
                ...
                if (selectedIncreseType === 1 || selectedIncreseType === "1") {
                    ...
                    $(mscobj).closest('tr').addClass("ss-opportunity");
                }
