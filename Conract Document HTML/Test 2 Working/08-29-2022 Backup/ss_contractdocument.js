        ...
        click: function () {
            ...
            if (tblLength > 0) {
                ...
                for (var i = 0; i < tblLength; i++) {
                    ...
                    entity.ss_increasedate = $(tr).find('td:eq(10)').find('input[type=date]').val();
  ...
  <entity name="opportunity">
    ...
    window.parent.Xrm.WebApi.online.retrieveMultipleRecords("opportunity", fetchXML2).then(
        function success(results) {
            ...
            myOppertutitiesList.forEach(element => {
                createContractDetailsRow(element.oppId, element.contracts, element.createdOn, element.ownerName, element.terminationTime, element.EndOfContract, element.renewal, element.contractid, element.priceincrease, element.ss_contractstatus, element.modifycontractstatus, element.cancellationatdate, element.cancellationtodate, element.contractcancelreason, element.ss_terminationperiod, element.contractowner, element.TL_annotationid, element.TL_filename, element.transfertofinance, element.contractnumber, element.ss_totallicencefee, element.salelsopportunityproductsobj, element.prdouctsname, element.ss_bcendofcontract, element.ss_currentyearpriceincrease, element.ss_increaseinpercentage, element.ss_increasedate, element.ss_revisionnumber);
            });
...
function CurrentYearPI(checkboxvalue) {
    if ($(checkboxvalue).prop("checked") == true) {
        $(checkboxvalue).closest('tr').find('.increasesTD').show();

    }
    else {
        $(checkboxvalue).closest('tr').find('.increasesTD').hide();
